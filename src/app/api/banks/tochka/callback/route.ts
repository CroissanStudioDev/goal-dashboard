...
export async function GET(request: NextRequest) {
  try {
    ...
    const _savedConsent = cookieStore.get('tochka_consent')?.value
    ...
    // Get accounts (exchangeCode already sets tokens internally)
    ...
    redirectUrl.searchParams.set(
      'success',
      `tochka_connected:${accounts.length}`,
    )
    return NextResponse.redirect(redirectUrl)
  } catch (error) {
    ...
    if (error instanceof Error && error.name === 'AuthError') {
      return NextResponse.redirect(
        new URL('/sign-in?callbackUrl=/settings', request.url),
      )
    }
